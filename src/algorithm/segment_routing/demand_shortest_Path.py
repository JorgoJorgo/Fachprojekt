...
class DemandShortestPath(GenericSR):
    BIG_M = 10 ** 9
        ...
        for d_map_idx in range(len(self.__demands)):
            ...
            if (sCache != s or tCache != t):
                ...
                for node in shortesPathNodeList :
                    for neigbor in self.__g.iterNeighbors(node) :
                        potentialWaypoints.add(neigbor)

            #jeden anderen Knoten (nicht source / target vom demand aus dem durchlauf) als möglichen waypoint nehmen
            for waypoint in  potentialWaypoints:
